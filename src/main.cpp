...
int main()
{
    ...
    auto mesh{assets.get_mesh("mask")};

    std::shared_ptr<fc::sphere_surface> surface{new fc::sphere_surface{{{0.0, 1.0, 0.0}}, 1.0}};
    std::shared_ptr<fc::mesh_surface> surface3{new fc::mesh_surface{{}, mesh}};
    ...
    //entities.push_back({surface, diffuse_material, nullptr});
    //entities.push_back({surface2, diffuse_material, nullptr});
    entities.push_back({surface3, diffuse_material, nullptr});


    //fc::brute_force_acceleration_structure_factory acceleration_structure_factory{};
